...
def devision(A: str, B: str, minusB: str, origB: str, k: int):
	print("Переход к основному циклу операции деления!")
	RQ = ['0' for i in range(len(A) * 2)]
	print(f"Значение регистра RQ = {''.join(RQ)}")
	Q = list()
	print("SUM. Операция NC = А + (-B)")
	print(f"{' ' * 23 + A}\n{' ' * 21}+\n{' ' * 23 + minusB}\n{' ' * 23 + '-' * len(minusB)}")
	ch_ost = plus_binary(A, minusB)
	print(f"Значение регистра NC = {ch_ost}")
	...
	print(f"Бит {int(len(RQ)/2-1)} регистра NC равен {ch_ost[0]},\nпоэтому {int(len(RQ)/2)} бита регистра RQ смещены влево на 1 бит "
		  f"с потерей 31-го, а бит 0 установлен в {RQ[-1]}")
	print(f"Значение регистра RQ = {''.join(RQ)}")
	for i in range(k): # 5.6
		...
		print(f"SHIFT. {int(len(RQ)/2)} бита регистра RQ смещены влево на 1 бит.\n"
			  f"Младший (0-ой) бит установлен в 0")
		if ch_ost[0] == '1': # 5.4
			print("SUM. Операция NC = NC + B")
			print(f"{' ' * 23 + ch_ost}\n{' ' * 21}+\n{' ' * 23 + B}\n{' ' * 23 + '-' * len(B)}")
		...
		# а частное дополняется нулями так, чтобы число разрядов частного равнялось k+1.
			break
	print(f"Значение {len(ch_ost)-1} разряда NC = {ch_ost[0]}")
	if ch_ost[0] == '1':
		print("В знаковом разряде содержится единица, значит требуется коррекция остатка.")
		print("SUM. Операция NC = NC + B")
		print(f"{' ' * 23 + ch_ost}\n{' ' * 21}+\n{' ' * 23 + B}\n{' ' * 23 + '-' * len(B)}")
		ch_ost = plus_binary(ch_ost, B)
		print(f"Значение регистра NC = {ch_ost}")
