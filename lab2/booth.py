...
def main():
	...
	sleep(1), print(f"Значение регистра P = {P}")
...
def cycle_Booth(A: str, S: str, P: str, iter: int):
	print(f"Вход в главный цикл вычислений алгоритма Бута\nКоличество итераций цикла = {iter}"), sleep(1)
	while iter > 0:
		if P[::-1][:2][::-1] == '01':
			print("ADD. Операция Р = А + Р"), sleep(1)
			print(f"{' ' * 22 + A}\n{' ' * 20}+\n{' ' * 22 + P}\n{' ' * 22 + '-' * len(S)}"), sleep(1)
			P = plus_binary(A, P)
			print(f"Значение регистра Р = {P}"), sleep(1)
		elif P[::-1][:2][::-1] == '10':
			print("SUB. Операция Р = S + Р"), sleep(1)
			print(f"{' ' * 22 + S}\n{' ' * 20}+\n{' ' * 22 + P}\n{' ' * 22 + '-' * len(S)}"), sleep(1)
			P = plus_binary(S, P)
			sleep(1)
			print(f"Значение регистра Р = {P}")
		else:
			print("NOP - действия над регистром Р не предпринимаются!"), sleep(1)
		P = shift_right(P)
		print(f"Операция P >> 1"), sleep(1), print(f"Значение регистра Р = {P}"), sleep(1)
		iter -= 1
	print("Выход из главного цикла вычислений алгоритма Бута"), sleep(1), print("Операция P >> 1"), sleep(1)
...
def ten_2_two(m: int, r: int, bits=4):
	...
	sleep(1), print("Представления введённых чисел в двоичном формате:")
	mask = (1 << bits) - 1
	if m < 0:
	...
	sleep(1), print(f"Множимое число m = {res_m}")
	if r < 0:
		res_r = (bin(((abs(r) ^ mask) + 1) & mask))[2:]
	else:
		res_r = bin(r)[2:]
		res_r = ''.join("0" for i in range(bits - len(res_r))) + res_r
	sleep(1), print(f"Число-множитель r = {res_r}"), sleep(1)
	return res_m, res_r
